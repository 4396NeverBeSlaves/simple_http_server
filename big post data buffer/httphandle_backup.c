...
{
    if (handle->read_rest_post_flag == -1) {
        goto process_post;
    } else if (handle->read_rest_post_flag > 0 && handle->read_rest_post_flag <= 5) {
        return read_post_data(handle);
    }
    ...
    if (handle->static_dynamic == STATIC_FILE) { //若是静态文档就直接读取文件再发送
        ...
        if (strcasecmp(method, "GET") == 0) {
            ...
            handle->has_read_post = read_count - (handle->read_ptr - handle->read_buf); //第一次读完1024字节后，去掉请求首部剩下需要读的post data的长度
            memcpy(handle->post_data, handle->read_ptr, handle->has_read_post);

            if (handle->has_read_post == handle->post_content_length) {
            process_post:
...
int read_post_data(httphandle* handle)
{
    int left, n;

    left = handle->post_content_length - handle->has_read_post; //在socket中还剩下的post data的数据长度
#ifdef _DEBUG
    printf("has_read_post:%d left:%d\n", handle->has_read_post, left);
#endif
    while (left > 0) {
        // while (1) {
        if ((n = recv(handle->fd, handle->post_data + handle->has_read_post, left, 0)) <= 0) {
            if (n == 0) {
                ...
                if (handle->has_read_post == handle->post_content_length) {
#ifdef _DEBUG
                    printf("errno==EAGAIN && has_read_post==post_content_length. recv end! break; \n");
                    fflush(stdout);
#endif
                    handle->read_rest_post_flag = -1;
                    break;
                } else {

                    if (handle->read_rest_post_flag < 5) {
                        handle->read_rest_post_flag++;
#ifdef _DEBUG
                        printf("errno==EAGAIN && read_rest_post_flag=%d NEED_READ \n", handle->read_rest_post_flag);
                        fflush(stdout);
#endif
                        return NEED_READ;
                    } else {
#ifdef _DEBUG
                        printf("errno==EAGAIN && read_rest_post_flag==5 NEED_READ \n");
                        fflush(stdout);
#endif
                        return NEED_DISCONNECT;
                    }
        ...
        handle->has_read_post += n;
        left -= n;
        // }
    }
    //     handle->post_data[has_read] = '\0';
    // #ifdef _DEBUG
    //     printf("total has_read:%d post_data:%.*s\n", has_read, 100, handle->post_data);
    //     fflush(stdout);
    // #endif
    return NEED_READ;
}
...
#ifdef _DEBUG
        printf("%int bytes read from cgi:%s\n", read_count, dynamic_file_buf);
        fflush(stdout);
#endif
...
int run_cgi_post(httphandle* handle, char* file_path)
{
    ...
    if ((pid = fork()) < 0) {
        ...
        int count, left = handle->post_content_length, has_written = 0;

        Close(pipe_send_to_child[0]);
        Close(pipe_recv_from_child[1]);

        while (left) {
            count = writen(pipe_send_to_child[1], handle->post_data + has_written, handle->post_content_length - has_written);
            has_written += count;
            left -= count;
        }
...
#ifdef _DEBUG
        printf("%d bytes read from cgi:%.*s\n", handle->send_file_size, 200, dynamic_file_buf);
        fflush(stdout);
#endif
        dynamic_doc_index = strstr(dynamic_file_buf, "\r\n\r\n");
        if (!dynamic_doc_index) {
